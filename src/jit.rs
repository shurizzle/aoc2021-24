...
impl Function {
  pub fn new(stmts: Vec<Statement>) -> Result<Self, ModuleError> {
...
pub fn compile(
  stmts: Vec<Statement>,
) -> Result<(JITModule, *const u8), ModuleError> {
  ...
  let id =
    module.declare_function("run", Linkage::Export, &ctx.func.signature)?;
...
fn translate(module: &mut JITModule, ctx: &mut Context, stmts: Vec<Statement>) {
  let int = I64;
...
fn get_reg_or_lit(
  ...
  match rol {
    RegisterOrLiteral::Register(reg) => {
      builder.use_var(variables[&VarKey::Reg(reg)])
    }
  ...
  I: Into<IntCC>,
{
  ...
  let cond_val = builder.ins().icmp(cc, a, b);
...
fn eql(
  ...
  cmp(IntCC::Equal, ty, variables, builder, a, b)
}
...
fn neq(
  ...
  cmp(IntCC::NotEqual, ty, variables, builder, a, b)
}
...
fn compile_op(
  ...
  match op {
    ...
    Statement::Set(a, b) => set(ty, variables, builder, a, b),
    Statement::Add(a, b) => add(ty, variables, builder, a, b),
    Statement::Mul(a, b) => mul(ty, variables, builder, a, b),
    Statement::Div(a, b) => div(ty, variables, builder, a, b),
    Statement::Mod(a, b) => rem(ty, variables, builder, a, b),
    Statement::Eql(a, b) => eql(ty, variables, builder, a, b),
    Statement::Neq(a, b) => neq(ty, variables, builder, a, b),
  }
}
...
#[cfg(test)]
fn test1_inner(
  next: unsafe extern "C" fn(*mut ()) -> *const (),
  data: *mut (),
) {
  loop {
